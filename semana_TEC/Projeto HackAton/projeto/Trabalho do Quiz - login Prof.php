...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $sql = "SELECT * FROM informa_professores WHERE nome = :nome AND email =:email AND senha= :senha";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(['nome' => $NickUSU, 'email' => $EmailUSU, 'senha' => $SenhaUSU]);
        $usuario = $stmt->fetch(PDO::FETCH_ASSOC);

        if($usuario){
            header("Location: Trabalho do Quiz - Menu.html");
            exit();
        }
        else{
            header("Location: Trabalho do Quiz - Tela Inicial.html");
            exit();
        }
